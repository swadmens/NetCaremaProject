...
@property (nonatomic, copy) void (^progressHandler)(CGFloat, CGFloat, NSString*);
...
- (instancetype)initWithUrl:(NSString *)urlStr progressHandler:(void (^)(CGFloat progress, CGFloat speed, NSString *writeBytes))progressHandler completionHandler:(void (^)(NSString *filePath, NSError *error))completionHandler
{
    if (self = [self init]) {
        //保存任务相关参数
        _downloadUrl = urlStr;
...
+ (instancetype)downloadTaskWithUrl:(NSString *)urlStr progressHandler:(void (^)(CGFloat progress, CGFloat speed, NSString *writeBytes))progressHandler completionHandler:(void (^)(NSString *filePath, NSError *error))completionHandler
{
...
- (NSURLSessionDataTask *)dataTask
{
    if (!_dataTask) {
        ...
        NSString *range = [NSString stringWithFormat:@"bytes=%lld-", _receivedLength];
...
- (void)startDownloadWithUrl:(NSString *)urlStr progressHandler:(void (^)(CGFloat progress, CGFloat speed, NSString *writeBytes))progressHandler completionHandler:(void (^)(NSString *filePath, NSError *error))completionHandler
{
    //保存任务相关参数
    _downloadUrl = urlStr;
    self.progressHandler = progressHandler;
    ...
    didReceiveData:(NSData *)data
{
    ...
    //文件大小下载显示
    NSString *writeBytes = [NSString stringWithFormat:@"%lldM/%lldM",_receivedLength/1048576,_expectedLength/1048576];
    ...
    if (self.progressHandler) {
        self.progressHandler(_taskProgress, _taskSpeed, writeBytes);
    }
}
