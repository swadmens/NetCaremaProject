...
- (void) showPopMenuSelecteWithFrame:(CGFloat)width
    ...
    [UIView animateWithDuration:0.3 animations:^{
        self.popMenuView.backImageView.transform = CGAffineTransformMakeScale(1.0, 1.0);
    }];
...
- (void) hideMenu {
    [UIView animateWithDuration:0.15 animations:^{
        self.popMenuView.backImageView.transform = CGAffineTransformMakeScale(0.0001, 0.0001);
    } completion:^(BOOL finished) {
        [self.popMenuView.backImageView removeFromSuperview];
        [self.popMenuView removeFromSuperview];
        self.popMenuView.backImageView = nil;
